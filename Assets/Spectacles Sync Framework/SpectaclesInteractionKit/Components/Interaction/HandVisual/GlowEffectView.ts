...
export class GlowEffectView {
  ...
  // TODO (LAF-4781) Set up tap events when we have a palm tap API
  ...
  /**
   * TODO (LAF-4745) Use HandInputData isInTargetingPose for this. Could not do it yet
   * because that API was not resulting in the behavior we wanted for this pinch
   * glow mechanic. To change that API will require careful planning so that we don't
   * break the places in the code where we are already using that API
   *
   * @returns boolean
   */
