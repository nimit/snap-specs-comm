...
class HeadlockRotationCalculator {
    ...
    calculateAngleDelta(axisVector, positionVector, originVector, forwardVector, upVector) {
        ...
        if (this.axis === HeadlockController_1.RotationAxis.Pitch && upVector !== undefined) {
            let forwardVectorOnXZ = new vec2(forwardVectorOnPlane.x, forwardVectorOnPlane.z).normalize();
            let positionVectorOnXZ = new vec2(positionVectorOnPlane.x, positionVectorOnPlane.z).normalize();
            let sameDirection = forwardVectorOnXZ.angleTo(positionVectorOnXZ) < Math.PI / 2;
        ...
        // Possibly not needed.
        let bufferAngle = angleDelta - Math.sign(angleDelta) * this.axisBufferRadians;
        return bufferAngle;
    }
    /**
    ...
    getOffset(axisVector, positionVector, originVector, forwardVector, upVector) {
        ...
        let angle = this.calculateAngleDelta(axisVector, positionVector, originVector, forwardVector, upVector);
        let rotationOffset = this.calculateRotationOffset(angle, axisVector, positionVector);
        return rotationOffset;
    }
